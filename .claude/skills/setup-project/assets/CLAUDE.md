# CLAUDE.md — Dự án web (Docusaurus + Cloudflare Pages)

Dự án này dựng bằng bộ 4 skill cho người dùng **non-tech**. File này nhắc agent các quy ước xuyên suốt cả 4 skill; chi tiết từng bước nằm trong `.claude/skills/<skill>/SKILL.md`.
...
- Deploy / redeploy: `bash .claude/skills/create-web/scripts/deploy.sh`
- Phân quyền: `node .claude/skills/access-control/scripts/sync-access.mjs [--dry-run|--prune]`
- Kiểm tra token: `bash .claude/skills/setup-project/scripts/verify-cloudflare.sh`
