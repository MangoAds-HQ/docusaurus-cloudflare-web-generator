...
Bước khởi tạo. Mục tiêu: kết thúc skill này, thư mục làm việc có một `site.config.json` hợp lệ + một file secret chứa token Cloudflare, và mọi prerequisite đã sẵn sàng để `create-web` chạy được ngay.
...
Khi người dùng đưa token, **lưu ngay vào file secret**, không in lại token ra màn hình, không ghi vào `site.config.json`:

```bash
cat > .cloudflare-secret <<EOF
CF_API_TOKEN=<token người dùng đưa>
CF_ACCOUNT_ID=<account id>
EOF
echo ".cloudflare-secret" >> .gitignore
chmod 600 .cloudflare-secret
```
...
curl -s "https://api.cloudflare.com/client/v4/user/tokens/verify" \
  -H "Authorization: Bearer $CF_API_TOKEN" | grep -o '"status":"[a-z]*"'
...
- File `.cloudflare-secret` phải nằm trong `.gitignore`.
