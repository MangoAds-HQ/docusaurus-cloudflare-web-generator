...
### Bước 5 — Sinh AGENT.md ở gốc dự án

Tạo `AGENT.md` ở gốc dự án (nếu chưa có) để mỗi phiên Claude sau này tự có context về quy ước cả 4 skill (vòng đời, file dùng chung, script, bảo mật). Copy từ template:

```bash
[ -f AGENT.md ] || cp .codex/skills/setup-project/assets/AGENT.md ./AGENT.md
```

> `AGENT.md` chỉ auto-load khi nằm ở gốc thư mục nơi chạy Claude — vì vậy đặt ở gốc dự án, KHÔNG để trong `.codex/`.
