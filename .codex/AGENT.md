# AGENT.md — Repo phát triển bộ skill dựng web
...
## Quy ước repo
- Source skill nằm ở `new-claude/skills/`; `.codex/skills/` là bản đang chạy. **Sửa ở `new-claude/` trước**, người dùng duyệt rồi mới copy sang `.codex/`.
- Khi sửa skill, giữ hai cây đồng bộ sau khi duyệt.

## Bộ skill (đúng thứ tự vòng đời)
1. **setup-project** — Cloudflare token + Zero Trust → sinh `.env` + `site.config.json` (+ `AGENT.md` cho project khách).
...
> Lưu ý: file này chỉ auto-load khi ở gốc thư mục nơi chạy Claude. Khi finalize, nếu muốn nó auto-load lúc dev, đặt một bản ở gốc project (vd `./AGENT.md`), không phải trong `.codex/`.
